...
var G = {
	...
	init : function(opts){
		...
		(loaderImage = _('img')).onload=function(){
			...
			imgture.setAttribute("src", loaderImage.src);
	...
	zoomNormal : function(){
		...
		G.Zoom(
			imgture.width == loaderImage.width && imgture.height == loaderImage.height ? 0 : 1
		);
	},
...
function setContPos(vScale, bIsOnload){
	...
	if (vScale == 0 || (bIsOnload && !vScale)) {
		...
		if (dLoadWidth > dAvailableWidth || dLoadHeight > dAvailableHeight) {
			...
			w = (imgture.width = newWidth) + dx;
			h = (imgture.height = newHeight) + dy;
		} else {
			w = (imgture.width = dLoadWidth) + dx;
			h = (imgture.height = dLoadHeight) + dy;
		}
		...
		w = (imgture.width *= vScale) + dx;
		h = (imgture.height *= vScale) + dy;
	}else{
	...
	} else if (imgture.width != dLoadWidth || imgture.height != dLoadHeight) {
		fsTitle = langVars.fullSize;
		fsClass = 'LG_zoomNormal';
	}

	fullSizeBtn.$disabled = false;
	if (imgture.width == dLoadWidth) {
	...
	var	scr = getScrollXY(),
		...
		// set the width of the prev/next buttons as 1/3 of the imgture width
		dBtnWidth = (w/3) + 'px',
	...
	return _(DIV, {id:'LG_container'},
			...
			_(DIV,{id:'LG_innerCont'},
				_(DIV, {id:'LG_panel'},
					...
					imgture = _('img', {id:'LG_img',width:300,height:300}),
